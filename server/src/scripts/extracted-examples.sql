// Example 1 [fire]
// Q: danh sách điểm cháy hôm nay với tọa độ và thông tin lô rừng
SELECT
...
// Example 2 [detect]
// Q: danh sách điểm biến động trong khoảng thời gian với thông tin xác minh
...
// Example 3 [detect]
// Q: thống kê điểm biến động theo chức năng rừng
WITH cte_verification AS (
...
// Example 4 [detect]
// Q: tìm lô rừng giao nhau với vùng phát hiện cháy
SELECT *,
...
// Example 5 [fire]
// Q: thống kê điểm cháy theo huyện trong tháng
SELECT
    fa.district_name,
...
// Example 6 [fire]
// Q: lấy ảnh điểm cháy từ camera cùng thông tin người xác minh
SELECT
...
// Example 7 [fire-camera]
// Q: camera nào phát hiện nhiều điểm cháy nhất tuần qua
SELECT
...
// Example 8 [detect]
// Q: số điểm biến động chưa xác minh theo xã
WITH latest_verification AS (
    SELECT DISTINCT ON (detect_id)
...
// Example 9 [fire]
// Q: xu hướng điểm cháy theo tuần trong năm
SELECT
...
// Example 10 [fire]
// Q: điểm cháy trong bán kính 5km từ tọa độ
SELECT
...
// Example 11 [fire]
// Q: tổng diện tích cháy theo loại rừng đã xác minh
SELECT
...
// Example 12 [detect]
// Q: thống kê điểm biến động theo nguyên nhân xác minh
WITH latest_verification AS (
    SELECT DISTINCT ON (detect_id)
        detect_id, cause_id, verified_at
    FROM detect.verification
...
// Example 13 [core]
// Q: lấy thông tin huyện xã của một user
SELECT
...
// Example 14 [detect]
// Q: thống kê điểm phát hiện từ vệ tinh trong kỳ
WITH cte_verification AS (
    SELECT detect_id, 1 AS is_verified
    FROM detect.verification
    GROUP BY detect_id
...
// Example 15 [fire]
// Q: lấy ảnh fire alert từ camera
SELECT
